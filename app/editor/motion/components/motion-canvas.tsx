    ...
    <div className="relative h-full w-full overflow-hidden rounded-[16px] bg-[#0a0a0a] text-white shadow-[0_32px_110px_rgba(0,0,0,0.45)]">
      <div
        ...
        onPointerDown={(event) => {
          ...
          if (isPanningRef.current) beginPan({ x: event.clientX, y: event.clientY })
        }}
        onPointerMove={(event) => {
          const point = toCanvasPoint({ x: event.clientX, y: event.clientY })
      ...
      >
        ...
        <div
          className="absolute left-0 top-0 h-[1800px] w-[2600px] origin-top-left transition-transform duration-200 ease-out"
          style={{
