'use client'

import { useCallback, useState, type ReactNode } from 'react'

import { cn } from '@/lib/utils'
import { CommandZone } from '@/components/editor/CommandZone'
import { EditorTopBar } from '@/components/editor/EditorTopBar'
import { FocusModeToggle } from '@/components/editor/FocusModeToggle'
import { KeyboardShortcuts } from '@/components/editor/KeyboardShortcuts'
import { SettingsPanel } from '@/components/editor/SettingsPanel'
import { EditorSidebarMediaDrawersV2 } from '@/components/editor/sidebar/editor-sidebar-media-drawers-v2'
import type { MusicDrawerProps } from '@/components/editor/music/music-drawer'

export interface EditorRouteShellMediaV2Props {
  children: ReactNode
  musicDrawerProps?: Omit<MusicDrawerProps, 'isOpen' | 'onClose'>
}

export function EditorRouteShellMediaV2({ children, musicDrawerProps }: EditorRouteShellMediaV2Props) {
  const [sidebarOpen, setSidebarOpen] = useState(true)
  const [focusMode, setFocusMode] = useState(false)
  const [settingsOpen, setSettingsOpen] = useState(false)
  const toggleSidebar = useCallback(() => setSidebarOpen((open) => !open), [])
  const toggleFocusMode = useCallback(() => setFocusMode((active) => !active), [])
  const closeOverlays = useCallback(() => setSettingsOpen(false), [])

  return (
    <div
      className={cn(
        'editor-root relative flex h-screen w-screen overflow-hidden bg-chrome-950 bg-chrome-radial text-text-primary',
        focusMode && 'prometheus-focus-mode'
      )}
      data-focus-mode={focusMode ? 'on' : 'off'}
    >
      <div className="pointer-events-none fixed inset-0 z-0 bg-chrome-radial" aria-hidden />
      <div id="ambient-orb-container" className="pointer-events-none fixed inset-0 z-0" aria-hidden />

      {!focusMode && sidebarOpen && (
        <EditorSidebarMediaDrawersV2
          defaultOpen
          onSidebarChange={setSidebarOpen}
          musicDrawerProps={musicDrawerProps}
          onNewProject={() => window.dispatchEvent(new CustomEvent('prometheus:editor:new-project'))}
        />
      )}

      <main className="relative z-10 flex min-w-0 flex-1 flex-col">
        {!focusMode && (
          <EditorTopBar
            onOpenSettings={() => setSettingsOpen(true)}
            onToggleSidebar={toggleSidebar}
            sidebarOpen={sidebarOpen}
          />
        )}
        <div className="min-h-0 flex-1 overflow-hidden">{children}</div>
        <CommandZone />
      </main>

      <FocusModeToggle active={focusMode} onToggle={toggleFocusMode} />
      <SettingsPanel
        focusMode={focusMode}
        onClose={() => setSettingsOpen(false)}
        onFocusModeChange={setFocusMode}
        open={settingsOpen}
      />
      <KeyboardShortcuts
        onCloseOverlays={closeOverlays}
        onToggleFocusMode={toggleFocusMode}
        onToggleSidebar={toggleSidebar}
      />
    </div>
  )
}
