...
const sidebarItems: SidebarItem[] = [
  ...
  { id: 'chat', label: 'Chat', icon: MessageSquare, action: 'panel', panel: 'chat' },
  { id: 'versions', label: 'Versions', icon: GitBranch, action: 'panel', panel: 'versions' },
  { id: 'status', label: 'Status', icon: Activity, action: 'panel', panel: 'status' },
  { id: 'export', label: 'Export', icon: Upload, action: 'panel', panel: 'export' },
    ...
    <>
      ...
      >
        <header className="flex items-center justify-between border-b border-white/10 px-4 py-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.32em] text-white/72">Prometheus</p>
        ...
        <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-3 py-4 scrollbar-hidden" aria-label="Editor tools">
          {sidebarItems.map((item) => {
            const Icon = item.icon
            const active = item.action === 'panel' ? activePanel === item.panel : pathname === ('href' in item ? item.href : '')
